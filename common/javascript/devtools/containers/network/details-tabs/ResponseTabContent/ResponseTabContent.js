...
const ResponseTabContent = ({ networkEvent }) => {
    ...
    const editResponseBody = useCallback(() => {
        createRule(RuleEditorUrlFragment.RESPONSE, (rule) => {
    ...
    const renderEditResponseBodyButton = useCallback(() => {
        if (isContentBodyEditable(networkEvent._resourceType)) {
            return (React.createElement(Button, { onClick: (e) => {
                    editResponseBody();
                    e.stopPropagation();
                }, icon: React.createElement(EditOutlined, null) }, "Edit Response Body"));
        }
        return (React.createElement(Tooltip, { title: "Only XHR/Fetch requests can be modified" },
            React.createElement(Button, { disabled: true, icon: React.createElement(EditOutlined, null) }, "Edit Response Body")));
    }, [networkEvent, response]);
    return (React.createElement("div", { className: "response-tab-content" },
        React.createElement(Collapse, { bordered: false },
            React.createElement(Collapse.Panel, { collapsible: "icon", key: "response-body", showArrow: false, header: "Response Body", extra: renderEditResponseBodyButton() })),
        React.createElement(CodeMirror, { basicSetup: {
                lineNumbers: true,
                syntaxHighlighting: true,
            }, theme: vscodeDark, value: response, height: "95%", extensions: editorExtensions, readOnly: true })));
};
