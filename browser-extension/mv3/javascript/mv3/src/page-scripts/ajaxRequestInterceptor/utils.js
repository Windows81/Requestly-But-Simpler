...
export const getFunctionFromCode = (code, ruleType) => {
    try {
        ...
        notifyOnErrorOccurred({
            ...
            if (!logShown) {
                logShown = true;
                console.log(`%cRequestly%c Please reload the page for ${ruleType} rule to take effect`, "color: #3c89e8; padding: 1px 5px; border-radius: 4px; border: 1px solid #91caff;", "color: red; font-style: italic");
            }
        });
        return () => { };
    }
...
const generateUserFunctionWithSharedState = function (functionStringEscaped) {
    ...
    const { func: generatedFunction, updatedSharedState } = new Function(`${SHARED_STATE_VAR_NAME}`, `return { func: ${functionStringEscaped}, updatedSharedState: ${SHARED_STATE_VAR_NAME}}`)(sharedState);
...
export const getMatchedRequestRule = (requestDetails) => {
    return window[PUBLIC_NAMESPACE]?.requestRules?.findLast((rule) => 
...
export const getMatchedResponseRule = (requestDetails) => {
    return window[PUBLIC_NAMESPACE]?.responseRules?.findLast((rule) => 
    // TODO: Move ruleMatcher outside of service worker
