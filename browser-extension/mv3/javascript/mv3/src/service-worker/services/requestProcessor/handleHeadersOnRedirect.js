...
export const forwardHeadersOnRedirect = async (tabId, requestDetails, rules) => {
    ...
    const ignoredHeaderValues = IGNORED_HEADERS_ON_REDIRECT.map((headerName) => {
        const headerKey = Object.keys(requestDetails.requestHeaders).find((key) => key.toLowerCase() === headerName.toLowerCase());
    ...
    return updateRequestSpecificRules(tabId, requestDetails.url, {
        action: {
            requestHeaders: ignoredHeaderValues.map((header) => ({
                header: header.name,
                value: header.value,
                operation: "set",
            })),
...
const checkIfHeaderExists = (requestHeaders, header) => {
