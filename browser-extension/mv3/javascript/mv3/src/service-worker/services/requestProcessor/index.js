...
class RequestProcessor {
    constructor() { }
    onBeforeAJAXRequest = async (tabId, requestDetails) => {
        const enabledRules = await rulesStorageService.getEnabledRules();
        if (enabledRules.length === 0) {
            return;
        }
        const redirectReplaceRules = enabledRules.filter((rule) => rule.ruleType === RuleType.REDIRECT || rule.ruleType === RuleType.REPLACE);
    ...
    onErrorOccurred = async (tabId, requestDetails) => {
        const enabledRules = await rulesStorageService.getEnabledRules();
        if (enabledRules.length === 0) {
            return;
        }
