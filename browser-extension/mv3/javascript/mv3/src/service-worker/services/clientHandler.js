...
const generateBlockedHostMatchPattern = (host) => {
    return [`*://${host}/*`, `*://*.${host}/*`];
};
...
const setupClientScript = async (isExtensionStatusEnabled) => {
...
const updateTabCache = async (tabId, obj, frameId) => {
    const target = {
        tabId: tabId,
    };
...
const updateTabRuleCache = async (tabId, frameId) => {
