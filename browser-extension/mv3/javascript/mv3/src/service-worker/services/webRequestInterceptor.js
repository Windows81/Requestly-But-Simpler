...
const onBeforeSendHeaders = async (details) => {
    let isMainOrPrerenderedFrame = details.type === "main_frame" || details.documentLifecycle === "prerender" ? true : false;
    if ((await isUrlInBlockList(details.initiator)) || (await isUrlInBlockList(details.url))) {
        return;
    }
    rulesStorageService.getEnabledRules().then((enabledRules) => {
...
const onHeadersReceived = async (details) => {
    let isMainOrPrerenderedFrame = 
    //@ts-ignore
    details.type === "main_frame" || details.documentLifecycle === "prerender" ? true : false;
    if ((await isUrlInBlockList(details.initiator)) || (await isUrlInBlockList(details.url))) {
...
export const addListeners = () => {
    ...
    if (!chrome.webRequest.onBeforeSendHeaders.hasListener(onBeforeSendHeaders)) {
        ...
        //@ts-ignore
        onBeforeSendHeaders, { urls: ["<all_urls>"] }, onBeforeSendHeadersOptions);
    }
    //@ts-ignore
    if (!chrome.webRequest.onHeadersReceived.hasListener(onHeadersReceived)) {
        ...
        //@ts-ignore
        onHeadersReceived, { urls: ["<all_urls>"] }, onHeadersReceivedOptions);
    }
};
...
export const initWebRequestInterceptor = () => {
    ...
    onVariableChange(Variable.IS_EXTENSION_ENABLED, (extensionStatus) => {
        if (extensionStatus) {
            addListeners();
        }
        else {
