...
import { AutoRecordingMode, SourceKey, SourceOperator } from "common/types";
...
const getSessionRecordingConfig = async (url) => {
    const sessionRecordingConfig = await getRecord(CONFIG_STORAGE_KEY);
...
export const initSessionRecordingSDK = async (tabId, frameId) => {
    await injectWebAccessibleScript("libs/requestly-web-sdk.js", {
        tabId,
        frameIds: [frameId],
    });
};
export const onSessionRecordingStartedNotification = (tabId, markIcon) => {
    if (markIcon) {
        extensionIconManager.markRecording(tabId);
    }
};
export const onSessionRecordingStoppedNotification = (tabId) => {
    extensionIconManager.markNotRecording(tabId);
};
export const getTabSession = (tabId, callback) => {
    chrome.tabs.sendMessage(tabId, { action: CLIENT_MESSAGES.GET_TAB_SESSION }, { frameId: 0 }, // top frame
    callback);
};
export const watchRecording = (tabId) => {
    chrome.tabs.create({ url: `${config.WEB_URL}/sessions/draft/${tabId}` });
};
const startRecording = (tabId, config) => {
    return chrome.tabs.sendMessage(tabId, {
        action: CLIENT_MESSAGES.START_RECORDING,
        payload: config,
    });
};
export const stopRecording = (tabId, openRecording) => {
...
export const startRecordingExplicitly = async (tab, showWidget = true) => {
...
export const launchUrlAndStartRecording = (url) => {
...
export const handleSessionRecordingOnClientPageLoad = async (tab, frameId) => {
...
export const cacheRecordedSessionOnClientPageUnload = (tabId, payload) => {
