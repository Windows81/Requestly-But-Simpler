...
const updateDynamicRules = async (options) => {
    const badRQRuleIds = new Set();
    while (true) {
        ...
        if (extensionRules?.length) {
            extensionRules.forEach((extensionRule) => {
                ...
                if (extensionRule.condition.resourceTypes?.length && extensionRule.condition.excludedResourceTypes?.length) {
                    extensionRule.condition.resourceTypes = extensionRule.condition.resourceTypes.filter((resourceType) => !extensionRule.condition.excludedResourceTypes.includes(resourceType));
                }
...
const applyExtensionRules = async () => {
...
export const initRulesManager = async () => {
...
export const updateRequestSpecificRules = async (tabId, requestUrl, ruleDetails, sessionRuleType) => {
