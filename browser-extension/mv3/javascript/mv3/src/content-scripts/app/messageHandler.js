...
const registerCallback = (message, callback) => {
    if (!callback)
        return;
...
const delegateMessageToBackground = (message) => {
    chrome.runtime.sendMessage(message, (bgResponse) => {
        sendResponse(message, bgResponse);
    });
};
export const sendExtensionMessage = (message, callback) => {
    if (!message.action) {
        return;
    }
    if (callback) {
...
const sendResponse = (originalEventData, response) => {
...
const clearAllRecordsExpectExtensionEnabledVariable = async () => {
    const isExtensionEnabled = await getVariable(Variable.IS_EXTENSION_ENABLED);
