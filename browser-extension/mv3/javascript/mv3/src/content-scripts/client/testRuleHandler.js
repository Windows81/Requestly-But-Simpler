...
let implictTestRuleWidgetConfig = null;
...
const showExplicitTestRuleWidget = async (ruleId) => {
...
const setWidgetInfoText = (testRuleWidget, ruleDetails) => {
    const { ruleType } = ruleDetails;
    const INFO_TEXT_ATTRIBUTE = "rq-test-rule-text";
    switch (ruleType) {
        case RuleType.RESPONSE:
            testRuleWidget.setAttribute(INFO_TEXT_ATTRIBUTE, `The responses are modified, but won't show in DevTools due to technical constraints. See <a class="link" target="_blank" href="https://docs.requestly.com/general/http-rules/advanced-usage/test-rules">docs</a> for details.`);
            break;
        case RuleType.HEADERS:
            ...
            responseHeaderExists &&
                testRuleWidget.setAttribute(INFO_TEXT_ATTRIBUTE, `Response Header Modifications will not show up in the browser network devtools due to technical constraints. Checkout docs for more <a target="_blank" href="https://docs.requestly.com/general/http-rules/rule-types/modify-headers">details</a>.`);
            break;
        default:
            return;
    }
};
const notifyRuleAppliedToExplicitWidget = (ruleId) => {
    ...
    if (explicitTestRuleWidget?.getAttribute("applied-status") === "false") {
        ...
        explicitTestRuleWidget.dispatchEvent(new CustomEvent("new-rule-applied", cloneDetails({
            detail: {
...
const notifyRuleAppliedToImplicitWidget = async (rule) => {
    ...
    if (implicitTestRuleWidget) {
        ...
        implicitTestRuleWidget.dispatchEvent(new CustomEvent("new-rule-applied", cloneDetails({
            detail: {
...
const fetchAndStoreImplicitTestRuleWidgetConfig = async () => {
    getRecord(STORAGE_KEYS.IMPLICIT_RULE_TESTING_WIDGET_CONFIG).then((result) => {
        if (result) {
            implictTestRuleWidgetConfig = result;
        }
    });
};
export const handleAppliedRuleNotification = async (rule) => {
...
const shouldShowImplicitWidget = (ruleType) => {
...
var ImplicitWidgetVisibility;
(function (ImplicitWidgetVisibility) {
    ImplicitWidgetVisibility["ALL"] = "all";
    ImplicitWidgetVisibility["SPECIFIC"] = "specific";
})(ImplicitWidgetVisibility || (ImplicitWidgetVisibility = {}));
