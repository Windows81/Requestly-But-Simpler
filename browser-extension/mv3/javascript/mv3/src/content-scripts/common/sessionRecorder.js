...
const sendStartRecordingEvent = async (sessionRecordingConfig) => {
    if (!sessionRecordingConfig) {
        return;
    }
    await initRecorder();
    const { notify, markRecordingIcon = true, explicit = false, recordingStartTime = Date.now(), showWidget, widgetPosition, previousSession, } = sessionRecordingConfig;
...
const sendResponseToRuntime = (action, payload) => {
    sendResponseCallbacks[action]?.(payload);
    delete sendResponseCallbacks[action];
};
const sendMessageToClient = (action, payload, sendResponseCallback) => {
...
const showManualModeRecordingWidget = () => {
    ...
    widget.dispatchEvent(new CustomEvent("show", cloneDetails({
        detail: {
