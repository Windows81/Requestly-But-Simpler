import { SourceKey, SourceOperator, RuleType, SourceFilterTypes, } from "common/types";
...
const createRegexForWildcardString = (wildCardString) => {
...
export const matchSourceUrl = (sourceObject, url) => {
...
const matchRequestWithRuleSourceFilters = function (sourceFilters, requestDetails) {
...
const matchRequestPayload = (requestPayloadFilter, requestData) => {
    if (!requestPayloadFilter)
        return true;
    if (typeof requestPayloadFilter === "object" && Object.keys(requestPayloadFilter).length === 0)
        return true;
...
export const matchRuleWithRequest = function (rule, requestDetails) {
    if (isBlacklistedURL(requestDetails.initiator) || isBlacklistedURL(requestDetails.url)) {
        return {};
    }
    const matchedPair = rule?.pairs?.find((pair) => matchSourceUrl(pair.source, requestDetails.url) &&
 ...
 */
export const populateMatchesInString = function (finalString, matches) {
...
export const populateRedirectedUrl = (rulePair, ruleType, requestDetails) => {
    switch (ruleType) {
...
export const findMatchingRule = (rules, requestDetails) => {
