RQ.SessionRecorder = class {
  ...
  static sendResponseToRuntime(action, payload) {
    SessionRecorder.sendResponseCallbacks[action]?.(payload);
    delete SessionRecorder.sendResponseCallbacks[action];
  }

  static sendMessageToClient(action, payload, sendResponseCallback) {
   ...
   */
  static bootstrapClient(namespace) {
  ...
  static showManualModeRecordingWidget() {
